...
This may come as a shock to some: but Raku really only knows about **binding** (`:=`) values.
...
> [Grokking](https://en.wikipedia.org/wiki/Grok#In_computer_programmer_culture) how containers work in Raku (as opposed to many other programming languages) is one of the rites of passage to becoming an experienced and efficient developer in Raku.  It sure was a rite of passage for yours truly!

In a simplified representation (almost pseudo-code) one can think of these `Scalar` objects like this:
```raku
class Scalar {
    has $!value;
    method STORE($value) {
...
> This type of information is kept in a so-called "container descriptor", which is considered to be an implementation detail.  More on this later.

And in the interest of performance, a lot of shortcuts are made in the implementation.  Because assignments are one of the very basic operations in a programming language.  And you want those to be fast!

Still, as a mental model this is very useful for understanding quite a few constructs in Raku.
...
Binding to containers allows for features in Raku that are considered "magic" by some, and maybe "too magic" by others.  But are part of common idioms in the Raku Programming Language.
...
say @a;  # [2 3 4 5 6]
```
Because the topic variable `$_` **binds** to the elements in the array, it represents the actual container at that location and can thus be incremented.
...
So there's no container yet for such an element.  Yet it is possible to assign to it!  How does that work?  The secret is really in the "container descriptor".  So let's refine our representation of a Scalar object:
```raku
class Scalar {
    has $!descriptor;
    has $!value;
...
```
Note that storing a value has now become a little more involved (`$!descriptor.process($value)` rather than just `$!value`).  It's this descriptor object (of which there are many different types in the Raku implementation, all starting with the `ContainerDescriptor::` name) that is responsible the name, the default type, type checking and a default value of a container.  And any additional logic that is needed.
...
These special containers for arrays and hashes also introduce an action-at-a-distance feature that you may or may not like.
```raku
my @a;
my $b := @a[3];
say @a;  # []
...
say %h;               # {a => {b => 42}}
```
So even though %h<a> is considered to be an `Associative` because of the `<b>`, it won't actually exist because of the `:exists` test.  Only after a value has been assigned, does `%h<a>` and `%h<a><b>` actually spring to life.
...
Inside the methods supplied you are completely free to put whatever code in there that you want.  As an example how that could work in a module, the [`Hash::MutableKeys`](https://raku.land/zef:lizmat/Hash::MutableKeys) distribution was created.  Another case of BDD (Blog Driven Development)!
...
In this episode the containers were described, as well as the special `Proxy` class that with its `STORE` and `FETCH` methods provides a fully customizable container implementation.
